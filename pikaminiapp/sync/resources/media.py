...
class MediaResource:
    """Resource for media-related API operations."""
    ...
    def create(
        ...
        *,
        ...
        data = self._http.post("/miniapp/media/create", payload)
    ...
    def iter_album(
        ...
        *,
        ...
        cursor = None
        while True:
            page = self.list_album(character_id, limit=page_size, cursor=cursor)
            yield from page.items
            if not page.has_more:
                break
            cursor = page.cursor

    def get_all_album_items(self, character_id: str) -> list[AlbumItem]:
        ...
        """
        return list(self.iter_album(character_id))
    ...
    def upload(
        ...
        *,
        ...
        Example:
            >>> result = client.media.upload("image.png")
            >>> print(result.url)  # CDN URL
            >>> # Later, add to album if needed:
            >>> client.media.add_to_album(character_id, result.media_id)
        """
        path = Path(file_path)
    ...
    def upload_bytes(
        ...
        *,
        ...
        Example:
            >>> result = client.media.upload_bytes(png_bytes, filename="gen.png")
            >>> print(result.url)  # CDN URL
        ...
        presign = self._get_presigned_upload(kind, unique_filename)

        # Upload to S3
        self._http.put_file(presign.presigned_url, content, content_type)

        # Create media asset
        media = self.create(presign.file_url, kind=kind, source=source)
    ...
    def upload_to_album(
        ...
        *,
        ...
        """
        result = self.upload(file_path, kind=kind, source=source)
        entry = self.add_to_album(character_id, result.media_id, captured=captured)
        result.album_id = entry.album_id
        return result

    def upload_bytes_to_album(
        self,
        character_id: str,
        content: bytes,
        *,
        ...
        Example:
            >>> result = client.media.upload_bytes_to_album(
    ...
    def _get_presigned_upload(self, file_type: str, filename: str) -> PresignedUpload:
